...
my_data1 = read.csv("buoyancy_gbr4_surface.csv")
...
p3 <- ggplot() +
  geom_point(my_data1, mapping = aes(x = Chl, y = Zoo, colour="GBR4-BGC-cyl"), alpha=0.1) + 
  geom_point(my_data, mapping = aes(x = Chl, y = Zoo, colour="GBR4-BGC-sph"), alpha=0.1) + 
  ...
  scale_color_manual("", breaks = c("GBR4-BGC-cyl", "GBR4-BGC-sph"), values = c("#0072BD", "#D95319"))



plt_data = ggarrange(p1, p2, ncol = 1, nrow = 2)
ggsave('phy_buoyancy.png',plt_data, width=7)

ggsave('zoo_buoyancy.png',p3)
