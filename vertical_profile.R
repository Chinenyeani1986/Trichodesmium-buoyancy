setwd("/export/home/a-e/cani/eReefs/buoyancy/tran_outputs1")
#library(dplyr)
library(ggplot2)
library('ereefs')
library(ncdf4)
...
profile_data = get_ereefs_profile(var_names= var_names,
			 ...
			 end_date = c(2010, 12, 20),
                         #input_file = "https://regional-models.ereefs.info/thredds/dodsC/GBR4_H2p0_B3p1_Cq3b_Dhnd/all/gbr4_bgc_all_simple_2012-10.nc",
...
p3 = plot_ereefs_zvt(profile_data, var_name='Tricho_I', scale_lim=NA, var_units= expression(mg~C~m^-3)) 
ggsave(p3, file= "tricho_I_prof.png" )

p4 = plot_ereefs_zvt(profile_data, var_name='Nfix', scale_lim=NA, var_units= expression(mg~N~m^-3~s^-1)) 
ggsave(p4, file= "Nfix_prof.png" )
